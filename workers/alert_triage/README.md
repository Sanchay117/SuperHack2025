...
-   **Retries** – If `_handle_message` raises an exception the message is left in the queue. Configure the queue with `RedrivePolicy` so that after _N_ receive attempts the message goes to `alert-triage-dlq`.
